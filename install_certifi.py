    ...
    if output.strip():
        ...
        if current_version:
            ...
            if current_version >= required_version:
                ...
                stdin, stdout, stderr = client.exec_command(f'python3 -m pip install --upgrade {package_name}=={required_version}')
                upgrade_output = stdout.read().decode()
                print(upgrade_output)
    else:
        print(f"[-] {package_name} not installed")
        print(f"[*] Installing {package_name}=={required_version}...")
        stdin, stdout, stderr = client.exec_command(f'python3 -m pip install {package_name}=={required_version}')
