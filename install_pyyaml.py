...
try:
    ...
    if 'not found' in error.lower() or 'WARNING' in output or output.strip() == '':
        print("[!] PyYAML not installed. Installing PyYAML==6.0.3...")
        
        # Install PyYAML 6.0.3
        stdin, stdout, stderr = client.exec_command('python3 -m pip install PyYAML==6.0.3')
        ...
        for line in output.split('\n'):
            if line.startswith('Version:'):
                ...
                print(f"[+] PyYAML already installed (version {version})")
        print(output)
