...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  const canonicalUrl = `${baseUrl}/all-tools/heic-to-png/${slug}`;
  const toolTitle = tool.title || tool.id.replace(/-/g, ' ').toUpperCase();

  return {
    title: `${toolTitle} - Free HEIC to PNG Converter Tool | SimplifyConvert`,
    description: tool.description || `Convert ${tool.from?.toUpperCase()} to ${tool.to?.toUpperCase()} easily.`,
    keywords: [toolTitle, 'heic to png', 'converter', 'free tool'],    robots: {
      index: true,
      follow: true,
    },    openGraph: {
      type: 'website',
      ...
      title: `${tool.title} - Free HEIC to PNG Converter`,
...
export default function HeicToPngSlugLayout({
  children,
}: {
