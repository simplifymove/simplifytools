...
export default function VHSEffectPage() {
    ...
    try {
      ...
      img.onload = async () => {
        try {
          ...
          // Apply VHS chromatic aberration and scanlines
          for (let i = 0; i < data.length; i += 4) {
            const idx = i / 4;
            const x = idx % canvas.width;
            const y = Math.floor(idx / canvas.width);

            // Random horizontal shift (chromatic effect)
            const shift = Math.floor((Math.random() - 0.5) * intensity * 2);
            const shiftIdx = Math.max(0, Math.min(i + shift * 4, data.length - 4));

            // Apply color degradation
            data[i] = (data[i] + data[shiftIdx]) / 2;
            data[i + 1] = (data[i + 1] + data[shiftIdx + 1]) / 2;
            data[i + 2] = (data[i + 2] + data[shiftIdx + 2]) / 2;

            // Scanline effect
            if (y % (3 + Math.floor(intensity / 2)) === 0) {
              data[i] *= 0.7;
              data[i + 1] *= 0.7;
              data[i + 2] *= 0.7;
            }
          ...
          // Add noise
          for (let i = 0; i < data.length; i += 4) {
            if (Math.random() < 0.02 * (intensity / 10)) {
      ...
      console.error('Error applying VHS effect:', error);
      setProcessing(false);
    }
  };
      ...
      try {
        const downloadResult =
          await uploadBrowserDownloadResult({
            blob: result,
            toolSlug: 'vhs-effect',
            originalName: `vhs-effect-${Date.now()}.jpg`,
            outputName: `vhs-effect-${Date.now()}.jpg`,
          });
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-slate-50 flex flex-col">
        {/* Hero Section */}
        <div className="bg-gradient-to-r from-red-500 via-pink-500 to-red-600 py-16 px-4 md:px-8">
          <div className="max-w-6xl mx-auto">
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">📼 VHS Effect</h1>
            <p className="text-lg text-white/90 max-w-2xl">
              Create retro VHS glitch and distortion effects. Transform photos into vintage video aesthetic.
            </p>
          </div>
        </div>
        ...
        <div className="flex-1 py-12 px-4 md:px-8">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">Apply VHS Effect</h2>
                ...
                <>
                  <div className="mb-6">
                    ...
                    <input
                      ...
                      className="w-full"
                    />
                    <p className="text-sm text-gray-600 mt-2">1 = subtle, 10 = intense VHS glitch</p>
                  </div>

                  <button
                    onClick={applyVHSEffect}
                    disabled={processing}
                  ...
                  >
                    ...
                    {processing ? 'Processing...' : 'Apply VHS Effect'}
                  </button>
                </>
              )}
            ...
            {/* How To Section */}
            <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">How to Apply VHS Effect</h2>
              <ol className="space-y-4 text-gray-700">
                <li><strong>1. Upload Photo:</strong> Select a JPG, PNG, or WebP image.</li>
                <li><strong>2. Adjust Intensity:</strong> Control VHS glitch level (1-10).</li>
                <li><strong>3. Apply Effect:</strong> Click "Apply VHS Effect".</li>
                <li><strong>4. Download:</strong> Save your retro VHS image.</li>
              </ol>
            </div>

            {/* Benefits Section */}
            <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">VHS Effect Benefits</h2>
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <h3 className="font-bold text-lg text-red-600 mb-2">Retro Aesthetic</h3>
                  <p className="text-gray-700">Create vintage VHS tape look.</p>
                </div>
                <div>
                  <h3 className="font-bold text-lg text-red-600 mb-2">Glitch Effects</h3>
                  <p className="text-gray-700">Add chromatic and distortion effects.</p>
                </div>
                <div>
                  <h3 className="font-bold text-lg text-red-600 mb-2">Customizable</h3>
                  <p className="text-gray-700">Control glitch intensity level.</p>
                </div>
                <div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
              ...
              <div className="space-y-4">
                <details className="border-l-4 border-red-500 pl-4 py-2">
                  ...
                  <p className="text-gray-700 mt-2">5 is balanced. Higher for more glitch artifacts.</p>
                </details>
                <details className="border-l-4 border-red-500 pl-4 py-2">
                  <summary className="font-bold text-gray-800 cursor-pointer">Works on all images?</summary>
                  <p className="text-gray-700 mt-2">JPG, PNG, and WebP images are supported by the upload control. Results vary by image.</p>
                </details>
                <details className="border-l-4 border-red-500 pl-4 py-2">
                  <summary className="font-bold text-gray-800 cursor-pointer">Can I use commercially?</summary>
                  <p className="text-gray-700 mt-2">Yes, VHS images can be used commercially.</p>
                </details>
                <details className="border-l-4 border-red-500 pl-4 py-2">
                  <summary className="font-bold text-gray-800 cursor-pointer">Output quality?</summary>
                  <p className="text-gray-700 mt-2">The processed result is exported as a JPEG with the VHS-style effect.</p>
                </details>
                <details className="border-l-4 border-red-500 pl-4 py-2">
                ...
                <details className="border-l-4 border-red-500 pl-4 py-2">
                  <summary className="font-bold text-gray-800 cursor-pointer">Realistic VHS?</summary>
                  <p className="text-gray-700 mt-2">Simulates authentic VHS glitch artifacts.</p>
                </details>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-8">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <Link href="/all-tools/glitch-effect" className="p-4 border-2 border-red-200 rounded-lg hover:bg-red-50 transition">
                ...
                <Link href="/all-tools/motion-blur" className="p-4 border-2 border-red-200 rounded-lg hover:bg-red-50 transition">
                  <h3 className="font-bold text-gray-800">Motion Blur</h3>
                  <p className="text-sm text-gray-600">Speed effects</p>
                </Link>
              </div>
            </div>
          </div>
        ...
        {/* Batch supporting content: vhs-effect */}
        <section className="bg-white border-t border-gray-200 px-4 md:px-8 py-14">
          <div className="max-w-6xl mx-auto space-y-10">

            <div>
              <h2 className="text-2xl font-bold text-gray-900 mb-3">
                How the VHS-style effect is created
              </h2>
              <p className="text-gray-600 leading-7">
                The VHS Effect modifies image pixels to introduce horizontal shifts, scan-line-style changes, and randomized visual artifacts. The intensity control changes how strongly these alterations appear. It is a visual simulation of a VHS aesthetic rather than restoration or processing of actual videotape footage.
              </p>
            </div>
      ...
      <script type="application/ld+json">
        {JSON.stringify({
          ...
          '@type': 'WebApplication',
          name: 'VHS Effect',
          description: 'Free online retro VHS glitch and vintage video effect tool',
