...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  const canonicalUrl = `${baseUrl}/all-tools/webp-to-tiff/${slug}`;
  const toolTitle = tool.title || tool.id.replace(/-/g, ' ').toUpperCase();

  return {
    title: `${toolTitle} - Free WebP to TIFF Converter Tool | SimplifyConvert`,
    description: tool.description || `Convert ${tool.from?.toUpperCase()} to ${tool.to?.toUpperCase()} easily.`,
    keywords: [toolTitle, 'webp to tiff', 'converter', 'free tool'],    robots: {
      index: true,
      follow: true,
    },    openGraph: {
      type: 'website',
      ...
      title: `${tool.title} - Free WebP to TIFF Converter`,
...
export default function WebpToTiffSlugLayout({
  children,
}: {
