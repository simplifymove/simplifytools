...
export const AUDIT_CATEGORY_DEFINITIONS: AuditCategoryDefinition[] = [
  ...
  },
  {
    ...
    tools: getAllPdfTools().map((tool) => withRoute(tool, `/all-tools/pdf/${tool.id}`, tool.id === 'add-text' ? {
      ...
      strategy: tool.id === 'edit-pdf' ? 'pdf-editor' : tool.id === 'annotate-pdf' ? 'pdf-annotate' : tool.id === 'esign-pdf' ? 'pdf-esign' : tool.inputMode === 'url' ? 'url' : 'file',
      ...
      processButtonText: tool.id === 'edit-pdf' ? '^Save$' : tool.id === 'annotate-pdf' ? 'Download Annotated PDF' : undefined,
    ...
    tools: [
      {
        ...
        functionalAudit: { strategy: 'text', textInput: 'SimplifyConvert functional audit sample.', resultFlow: 'download', expectedOutput: outputContract('.mp3'), executionClass: 'LOCAL_DETERMINISTIC' },
      },
    ],
  },
];
