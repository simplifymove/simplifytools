...
export async function readDownloadResultResponse(response: Response): Promise<Required<DownloadResultResponse>> {
  const result = await response.json() as DownloadResultResponse & { error?: string };
  if (!response.ok) {
...
export async function uploadBrowserPdfResult({
  ...
  return readDownloadResultResponse(await fetch('/api/pdf/browser-result', {
    method: 'POST',
    body: formData,
