...
try:
    ...
    stdin, stdout, stderr = client.exec_command('python3 -m pip show scipy')
    output = stdout.read().decode()
    error = stderr.read().decode()
    
    if 'not found' in error.lower() or 'WARNING' in output or output.strip() == '':
        print("[!] scipy not installed. Installing scipy==1.17.1...")
        
        # Install scipy 1.17.1
        stdin, stdout, stderr = client.exec_command('python3 -m pip install scipy==1.17.1')
        ...
        stdin, stdout, stderr = client.exec_command('python3 -c "import scipy; print(f\'scipy version: {scipy.__version__}\')"')
        ...
        for line in output.split('\n'):
            if line.startswith('Version:'):
                ...
                print(f"[+] scipy already installed (version {version})")
        print(output)
